...
-- finalResult	=>  Location: PIN_AE25,	 I/O Standard: 3.3-V LVTTL,	 Current Strength: 24mA
...
-- challenge[3]	=>  Location: PIN_P26,	 I/O Standard: 3.3-V LVTTL,	 Current Strength: Default
...
-- pragma translate_on
PORT MAP (
	datab => \ro1|inst_delay_lines:3:inst_delay_line|andg|output~combout\,
...
-- Equation(s):
-- \c2|count[1]~7_combout\ = (\c2|count\(0) & (\c2|count\(1) $ (VCC))) # (!\c2|count\(0) & (\c2|count\(1) & VCC))
-- \c2|count[1]~8\ = CARRY((\c2|count\(0) & \c2|count\(1)))
...
-- Equation(s):
-- \c1|count[1]~7_combout\ = (\c1|count\(0) & (\c1|count\(1) $ (VCC))) # (!\c1|count\(0) & (\c1|count\(1) & VCC))
-- \c1|count[1]~8\ = CARRY((\c1|count\(0) & \c1|count\(1)))
...
-- Equation(s):
-- \comp|LessThan0~1_cout\ = CARRY((\c2|count\(0) & !\c1|count\(0)))
...
-- Equation(s):
-- \comp|LessThan0~7_cout\ = CARRY((\c2|count\(3) & (\c1|count\(3) & !\comp|LessThan0~5_cout\)) # (!\c2|count\(3) & ((\c1|count\(3)) # (!\comp|LessThan0~5_cout\))))
